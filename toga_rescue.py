#!/usr/bin/env python3
"""
Rescue script for completing toga after cesar failed.
Run after finishing cesar manually
"""
import argparse
import sys
import os
...
class Toga:
    ...
    def __run_cesar_jobs(self):
        ...
        for b in buckets:
            ...
            if not self.local_executor and not self.uge and not self.para:
            ...
            if not self.para and not self.uge:  # create nextflow cmd
        ...
        # push bigmem jobs
        if self.nextflow_bigmem_config and not self.para and not self.uge:
            ...
            if big_lines_num == 0:
                ...
                if self.exec_cesar_parts_sequentially:
                    p.wait()
                processes.append(p)
        elif self.para and self.para_bigmem and not self.uge:
    ...
    def __toga_recap(self):
        """set all the values that were originally set in the previous steps"""
        self.ch_cl_jobs = os.path.join(self.temp_wd, "chain_classification_jobs")
        # for raw results of this stage
        self.chain_class_results = os.path.join(
            self.temp_wd, "chain_classification_results"
